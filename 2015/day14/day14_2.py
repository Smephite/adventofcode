...
for t in range(0, 2503):
    ...
    best_dist = max(map(lambda x : reindeers[str(x)]['dist_traveled'], reindeers))

    for winning_deer in filter(lambda deer: reindeers[deer]['dist_traveled'] == best_dist, reindeers):
        scores[winning_deer] = scores[winning_deer] + 1

print("The winning reindeer has ", max(scores.values()), "points")